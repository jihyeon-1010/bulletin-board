package com.example.bulletinboardapp.config.handler.oauth;
...
public class OAuth2SuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
    ...
    public static final Duration REFRESH_TOKEN_DURATION = Duration.ofDays(14); // 리프레시 토큰의 유효 기간(14일)
    ...
    public void onAuthenticationSuccess(
        ...
        CustomOAuth2User oAuth2Menber = (CustomOAuth2User) authentication.getPrincipal();
        User user = userService.findByEmail(oAuth2Menber.getEmail());

        // 토큰 제공자를 사용해 리프레시 토큰 생성
        String refreshToken = tokenProvider.generateToken(user, REFRESH_TOKEN_DURATION);
        // 해당 리프레시 토큰을 유저 id와 함께 db에 저장
        saveRefreshToken(user.getId(), refreshToken);
        // 클라이언트에서 액세스 토큰이 만료되면 재발급 요청 후 쿠키에 리프레시 토큰 저장
